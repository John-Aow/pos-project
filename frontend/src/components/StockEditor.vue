<template>
  <section class="menu-editor" aria-labelledby="stock-editor-title">
    <header class="menu-editor__header">
      <div>
        <p class="menu-editor__eyebrow">Stock</p>
        <h3 id="stock-editor-title" class="menu-editor__title">Update item stock</h3>
      </div>
    ...
    <p class="menu-editor__summary">
      {{ item ? `${item.name} is ready for a new quantity.` : 'Pick an item from the menu.' }}
    </p>

    <form class="menu-editor__form" @submit.prevent="submit">
      <label class="menu-editor__field">
        <span class="menu-editor__label">Stock quantity</span>
        <input
          :value="draft"
          class="menu-editor__input"
          data-testid="stock-editor-input"
          min="0"
          step="1"
          type="number"
          @input="updateDraft"
